...
st.write("""
    ...
    This application predicts the likelihood of motor failure based on accelerometer data inputs. Enter the **X**, **Y**, and **Z** values from the accelerometer to get insights into the motor’s performance.
    
    📊 **How it Works:**
    1. Enter the accelerometer values in the input fields.
...
x_input = st.number_input("**X Value**", format="%.5f", help="Input the X-axis value from the accelerometer.")
y_input = st.number_input("**Y Value**", format="%.5f", help="Input the Y-axis value from the accelerometer.")
z_input = st.number_input("**Z Value**", format="%.5f", help="Input the Z-axis value from the accelerometer.")
